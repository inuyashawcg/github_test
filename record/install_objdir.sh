#! /bin/bash

root_path="/home/mercury/Documents/code/qihai/master/qihai/freebsd-src"
world_dir="${root_path}/obj/riscv"

install_objdir=(
  "sbin"
  "libexec"
  "gnu"
  "kerberos5"
  "include"
  "secure"
  "usr.sbin"
  "stand"
  "sys"
  "tests"
  "etc"
  "rescue/librescue"
  "kerberos5/lib"
  "gnu/lib"
  "secure/lib"
  "gnu/usr.bin"
  "stand/libsa"
  "share/colldef"
  "tests/etc"
  "gnu/lib/libdialog"
  "gnu/usr.bin/dialog"
  "secure/lib/libcrypto"
  "libexec/atf"
  "bin/cat"
  "kerberos5/lib/libasn1"
  "etc/sendmail"
  "libexec/atf/atf-check"
  "include/arpa"
  "sbin/adjkerntz"
  "gnu/usr.bin/diff3"
  "tests/etc/rc.d"
  "gnu/lib/tests"
  "bin/cat/tests"
  "sbin/camcontrol"
  "lib/csu"
  "libexec/atf/atf-check/tests"
  "stand/fdt"
  "include/protocols"
  "include/rpcsvc"
  "lib/csu/riscv"
  "gnu/usr.bin/tests"
  "tests/sys"
  "bin/chflags"
  "sbin/clri"
  "gnu/tests"
  "sbin/comcontrol"
  "libexec/atf/atf-sh"
  "usr.sbin/adduser"
  "stand/ficl"
  "lib/csu/tests"
  "bin/chflags/tests"
  "include/rpc"
  "sbin/conscontrol"
  "secure/libexec"
  "kerberos5/lib/libhdb"
  "kerberos5/lib/libheimntlm"
  "sbin/ddb"
  "secure/libexec/sftp-server"
  "usr.sbin/arp"
  "libexec/atf/atf-sh/tests"
  "usr.bin/alias"
  "tests/sys/pjdfstest"
  "bin/chio"
  "stand/forth"
  "tests/sys/common"
  "lib/csu/tests/dso"
  "tests/sys/pjdfstest/pjdfstest"
  "sbin/devfs"
  "secure/libexec/ssh-keysign"
  "include/xlocale"
  "usr.sbin/binmiscctl"
  "kerberos5/lib/libhx509"
  "kerberos5/lib/libkadm5clnt"
  "usr.bin/apply"
  "libexec/atf/tests"
  "bin/chmod"
  "lib/csu/tests/dynamic"
  "tests/sys/pjdfstest/tests"
  "tests/sys/acl"
  "stand/liblua"
  "sbin/devmatch"
  "secure/libexec/ssh-pkcs11-helper"
  "libexec/atrun"
  "include/gssapi"
  "usr.sbin/camdd"
  "bin/chmod/tests"
  "usr.bin/apply/tests"
  "kerberos5/lib/libkadm5srv"
  "lib/csu/tests/dynamiclib"
  "secure/libexec/ssh-sk-helper"
  "sbin/dhclient"
  "stand/lua"
  "tests/sys/aio"
  "tests/sys/pjdfstest/tests/chflags"
  "kerberos5/lib/libkrb5"
  "kerberos5/lib/libroken"
  "usr.sbin/cdcontrol"
  "libexec/blacklistd-helper"
  "bin/cp"
  "sbin/dhclient/tests"
  "secure/libexec/tests"
  "lib/csu/tests/dynamicpie"
  "usr.bin/asa"
  "kerberos5/lib/libsl"
  "tests/sys/pjdfstest/tests/chmod"
  "stand/defaults"
  "usr.sbin/chkgrp"
  "libexec/comsat"
  "sbin/dmesg"
  "lib/csu/tests/static"
  "bin/cp/tests"
  "secure/usr.bin"
  "kerberos5/lib/libvers"
  "kerberos5/lib/libkdc"
  "tests/sys/pjdfstest/tests/chown"
  "usr.sbin/chown"
  "stand/fonts"
  "libexec/dma"
  "sbin/dump"
  "secure/usr.bin/openssl"
  "bin/date"
  "usr.bin/backlight"
  "lib/libc"
  "libexec/dma/dmagent"
  "lib/libc_nonshared"
  "kerberos5/lib/libwind"
  "kerberos5/lib/libheimbase"
  "obj/include"
  "tests/sys/pjdfstest/tests/ftruncate"
  "sbin/dumpfs"
  "usr.sbin/chown/tests"
  "secure/lib/libssl"
  "stand/images"
  "bin/date/tests"
  "usr.bin/banner"
  "secure/usr.bin/scp"
  "libexec/dma/dma-mbox-create"
  "lib/libclang_rt"
  "sbin/dumpon"
  "kerberos5/lib/libheimipcc"
  "usr.sbin/chroot"
  "tests/sys/pjdfstest/tests/granular"
  "lib/libclang_rt/asan"
  "stand/man"
  "usr.bin/basename"
  "bin/dd"
  "secure/lib/libssh"
  "secure/usr.bin/sftp"
  "libexec/flua"
  "secure/lib/libcrypto/engines"
  "sbin/etherswitchcfg"
  "tests/sys/pjdfstest/tests/link"
  "usr.bin/basename/tests"
  "kerberos5/lib/libheimipcs"
  "usr.sbin/ckdist"
  "secure/lib/libcrypto/engines/capi"
  "bin/dd/tests"
  "lib/libclang_rt/asan/asan"
  "lib/libclang_rt/asan/interception"
  "lib/libclang_rt/asan/sanitizer_common"
  "lib/libclang_rt/asan/ubsan"
  "secure/usr.bin/ssh"
  "lib/libc++"
  "stand/efi"
  "lib/libclang_rt/asan-preinit"
  "libexec/getty"
  "sbin/ffsinfo"
  "lib/libclang_rt/asan-preinit/asan"
  "stand/efi/libefi"
  "usr.sbin/clear_locks"
  "kerberos5/lib/libkafs5"
  "secure/usr.sbin"
  "secure/usr.bin/ssh-add"
  "secure/lib/tests"
  "lib/libclang_rt/asan_cxx"
  "usr.bin/beep"
  "bin/df"
  "tests/sys/pjdfstest/tests/mkdir"
  "secure/usr.sbin/sshd"
  "libexec/mail.local"
  "sbin/fsck"
  "lib/libc++/filesystem"
  "lib/libclang_rt/asan_cxx/asan"
  "kerberos5/lib/libgssapi_krb5"
  "lib/libclang_rt/asan_cxx/ubsan"
  "stand/efi/fdt"
  "usr.sbin/crashinfo"
  "kerberos5/lib/libgssapi_ntlm"
  "usr.bin/brandelf"
  "bin/domainname"
  "secure/usr.bin/ssh-agent"
  "lib/libclang_rt/asan_dynamic"
  "tests/sys/pjdfstest/tests/mkfifo"
  "libexec/makewhatis.local"
  "sbin/fsck_ffs"
  "secure/usr.sbin/tests"
  "usr.sbin/cron"
  "tests/sys/pjdfstest/tests/mknod"
  "kerberos5/lib/libgssapi_spnego"
  "usr.bin/bsdcat"
  "secure/usr.bin/ssh-keygen"
  "bin/echo"
  "libexec/phttpget"
  "usr.sbin/cron/lib"
  "stand/efi/boot1"
  "lib/libclang_rt/asan_dynamic/asan"
  "secure/tests"
  "lib/libclang_rt/asan_dynamic/interception"
  "lib/libclang_rt/asan_dynamic/sanitizer_common"
  "lib/libclang_rt/ubsan_minimal"
  "lib/libclang_rt/asan_dynamic/ubsan"
  "sbin/fsck_msdosfs"
  "sbin/fsdb"
  "lib/libclang_rt/ubsan_standalone"
  "bin/echo/tests"
  "usr.bin/bsdcat/tests"
  "kerberos5/libexec"
  "libexec/pppoed"
  "tests/sys/pjdfstest/tests/open"
  "usr.sbin/cron/cron"
  "secure/usr.bin/ssh-keyscan"
  "lib/libclang_rt/ubsan_minimal/ubsan_minimal"
  "secure/caroot"
  "sbin/fsirand"
  "stand/efi/gptboot"
  "lib/libclang_rt/ubsan_standalone_cxx"
  "sbin/gbde"
  "kerberos5/libexec/digest-service"
  "secure/caroot/trusted"
  "bin/ed"
  "lib/libclang_rt/ubsan_standalone/interception"
  "libexec/rc"
  "lib/libclang_rt/ubsan_standalone/sanitizer_common"
  "lib/libclang_rt/ubsan_standalone/ubsan"
  "secure/usr.bin/tests"
  "usr.bin/bsdiff"
  "usr.sbin/cron/crontab"
  "lib/libclang_rt/profile"
  "tests/sys/pjdfstest/tests/rename"
  "sbin/geom"
  "usr.bin/bsdiff/bsdiff"
  "stand/efi/loader_4th"
  "lib/libclang_rt/ubsan_standalone_cxx/ubsan"
  "usr.sbin/daemon"
  "sbin/ggate"
  "usr.bin/bsdiff/bspatch"
  "bin/expr"
  "kerberos5/libexec/ipropd-master"
  "lib/libclang_rt/profile/profile"
  "kerberos5/libexec/ipropd-slave"
  "libexec/rc/rc.d"
  "sbin/growfs"
  "secure/caroot/blacklisted"
  "sbin/ggate/ggatec"
  "tests/sys/pjdfstest/tests/rmdir"
  "lib/libc++experimental"
  "kerberos5/libexec/hprop"
  "usr.sbin/daemon/tests"
  "stand/efi/loader_lua"
  "kerberos5/libexec/hpropd"
  "bin/expr/tests"
  "usr.bin/bzip2"
  "lib/libcxxrt"
  "kerberos5/libexec/kdc"
  "sbin/growfs/tests"
  "tests/sys/pjdfstest/tests/symlink"
  "sbin/ggate/ggated"
  "sbin/gvinum"
  "usr.sbin/dconschat"
  "usr.sbin/devctl"
  "bin/freebsd-version"
  "usr.bin/bzip2recover"
  "kerberos5/libexec/kdigest"
  "stand/efi/loader_simp"
  "kerberos5/libexec/kfd"
  "sbin/ggate/ggatel"
  "lib/libelf"
  "sbin/ifconfig"
  "tests/sys/pjdfstest/tests/truncate"
  "usr.sbin/devinfo"
  "sbin/init"
  "libexec/revnetgroup"
  "usr.sbin/diskinfo"
  "usr.bin/cap_mkdb"
  "bin/getfacl"
  "kerberos5/libexec/kimpersonate"
  "kerberos5/tools"
  "kerberos5/libexec/kpasswdd"
  "sbin/kldconfig"
  "sbin/ifconfig/tests"
  "lib/libelf/sys"
  "libexec/rpc.rquotad"
  "sbin/kldload"
  "usr.sbin/dumpcis"
  "tests/sys/pjdfstest/tests/unlink"
  "usr.bin/chat"
  "lib/libssp"
  "tests/sys/pjdfstest/tests/utimensat"
  "kerberos5/tools/make-roken"
  "bin/hostname"
  "kerberos5/libexec/kcm"
  "sbin/kldstat"
  "tests/sys/audit"
  "sbin/kldunload"
  "sbin/ldconfig"
  "bin/kenv"
  "libexec/rpc.rstatd"
  "kerberos5/tools/asn1_compile"
  "tests/sys/auditpipe"
  "usr.bin/chpass"
  "usr.sbin/etcupdate"
  "bin/kill"
  "kerberos5/libexec/kadmind"
  "sbin/md5"
  "lib/msun"
  "sbin/mdconfig"
  "tests/sys/capsicum"
  "sbin/mdmfs"
  "bin/ln"
  "kerberos5/tools/slc"
  "tests/sys/devrandom"
  "libexec/rpc.rusersd"
  "usr.bin/cksum"
  "usr.sbin/etcupdate/tests"
  "lib/libsqlite3"
  "bin/ls"
  "sbin/mdconfig/tests"
  "sbin/mknod"
  "sbin/md5/tests"
  "tests/sys/fifo"
  "sbin/mksnap_ffs"
  "usr.bin/cmp"
  "bin/ls/tests"
  "bin/ln/tests"
  "libexec/rpc.rwalld"
  "kerberos5/usr.bin"
  "usr.sbin/extattr"
  "tests/sys/file"
  "usr.sbin/extattrctl"
  "lib/geom"
  "sbin/mount"
  "sbin/mount_cd9660"
  "tests/sys/fs"
  "bin/mkdir"
  "usr.bin/cmp/tests"
  "lib/geom/cache"
  "kerberos5/usr.bin/hxtool"
  "usr.sbin/extattr/tests"
  "lib/msun/tests"
  "usr.sbin/fifolog"
  "bin/mv"
  "libexec/rpc.sprayd"
  "tests/sys/geom"
  "sbin/mount_fusefs"
  "usr.sbin/fifolog/lib"
  "sbin/mount_msdosfs"
  "sbin/mount_nfs"
  "bin/mkdir/tests"
  "usr.bin/col"
  "usr.sbin/fstyp"
  "tests/sys/fs/fusefs"
  "lib/geom/concat"
  "kerberos5/usr.bin/kadmin"
  "libexec/save-entropy"
  "lib/lib9p"
  "usr.sbin/fifolog/fifolog_create"
  "bin/mv/tests"
  "sbin/mount_nullfs"
  "sbin/mount_udf"
  "sbin/mount_unionfs"
  "tests/sys/geom/class"
  "usr.bin/col/tests"
  "sbin/newfs"
  "lib/lib9p/backend"
  "lib/geom/eli"
  "lib/lib9p/transport"
  "libexec/smrsh"
  "usr.sbin/fstyp/tests"
  "lib/libalias"
  "tests/sys/fs/tmpfs"
  "usr.sbin/fifolog/fifolog_writer"
  "usr.sbin/fifolog/fifolog_reader"
  "sbin/newfs_msdos"
  "kerberos5/usr.bin/kcc"
  "sbin/nfsiod"
  "bin/pax"
  "lib/libalias/libalias"
  "lib/geom/journal"
  "usr.bin/colrm"
  "tests/sys/geom/class/concat"
  "libexec/telnetd"
  "tests/sys/geom/class/eli"
  "usr.sbin/fwcontrol"
  "libexec/tests"
  "tests/sys/kern"
  "sbin/newfs_msdos/tests"
  "kerberos5/usr.bin/kdestroy"
  "sbin/nos-tun"
  "lib/libalias/modules"
  "sbin/pfilctl"
  "lib/geom/label"
  "bin/pax/tests"
  "bin/pkill"
  "sbin/rcorder"
  "lib/libalias/modules/dummy"
  "usr.sbin/getfmac"
  "usr.bin/column"
  "tests/sys/geom/class/gate"
  "libexec/tftp-proxy"
  "sbin/reboot"
  "tests/sys/kern/acct"
  "sbin/recoverdisk"
  "kerberos5/usr.bin/kgetcred"
  "sbin/resolvconf"
  "bin/ps"
  "bin/pkill/tests"
  "lib/geom/mirror"
  "sbin/restore"
  "libexec/ulog-helper"
  "lib/libalias/modules/ftp"
  "usr.bin/comm"
  "sbin/route"
  "usr.sbin/getpmac"
  "tests/sys/geom/class/mirror"
  "sbin/savecore"
  "bin/pwait"
  "tests/sys/kern/execve"
  "lib/geom/mountver"
  "kerberos5/usr.bin/kf"
  "sbin/setkey"
  "kerberos5/usr.sbin"
  "bin/pwd"
  "libexec/bootpd"
  "usr.sbin/gstat"
  "usr.bin/comm/tests"
  "lib/libalias/modules/irc"
  "kerberos5/usr.sbin/iprop-log"
  "sbin/route/tests"
  "tests/sys/geom/class/multipath"
  "kerberos5/usr.sbin/kstash"
  "lib/geom/multipath"
  "bin/pwait/tests"
  "kerberos5/usr.bin/kinit"
  "tests/sys/kern/pipe"
  "sbin/shutdown"
  "usr.sbin/i2c"
  "usr.bin/compress"
  "bin/realpath"
  "libexec/bootpd/bootpgw"
  "lib/libalias/modules/nbt"
  "sbin/spppcontrol"
  "sbin/swapon"
  "tests/sys/geom/class/nop"
  "kerberos5/usr.bin/kpasswd"
  "kerberos5/usr.bin/krb5-config"
  "lib/geom/nop"
  "tests/sys/kqueue"
  "kerberos5/usr.sbin/ktutil"
  "usr.sbin/ifmcstat"
  "lib/geom/part"
  "usr.bin/compress/tests"
  "bin/rm"
  "libexec/bootpd/tools"
  "lib/libalias/modules/pptp"
  "sbin/sysctl"
  "tests/sys/geom/class/part"
  "sbin/tunefs"
  "libexec/bootpd/tools/bootpef"
  "lib/geom/raid"
  "kerberos5/usr.bin/ksu"
  "tests/sys/kqueue/libkqueue"
  "kerberos5/usr.bin/string2key"
  "usr.sbin/iostat"
  "lib/geom/raid3"
  "usr.sbin/iovctl"
  "usr.bin/cpuset"
  "lib/libalias/modules/skinny"
  "bin/rm/tests"
  "sbin/ping"
  "tests/sys/mac"
  "sbin/umount"
  "tests/sys/geom/class/raid3"
  "libexec/bootpd/tools/bootptest"
  "lib/geom/shsec"
  "kerberos5/usr.bin/verify_krb5_conf"
  "usr.sbin/kldxref"
  "lib/libalias/modules/smedia"
  "bin/rmdir"
  "lib/libarchive"
  "usr.bin/csplit"
  "usr.sbin/mailwrapper"
  "tests/sys/geom/class/shsec"
  "tests/sys/mac/bsdextended"
  "sbin/ccdconfig"
  "lib/geom/stripe"
  "lib/libauditd"
  "sbin/ping/tests"
  "libexec/fingerd"
  "usr.bin/csplit/tests"
  "tests/sys/geom/class/stripe"
  "bin/rmdir/tests"
  "usr.sbin/makefs"
  "lib/libbegemot"
  "usr.sbin/memcontrol"
  "tests/sys/geom/class/uzip"
  "sbin/devd"
  "tests/sys/mac/portacl"
  "lib/geom/virstor"
  "sbin/hastctl"
  "libexec/ftpd"
  "libexec/rbootd"
  "libexec/talkd"
  "bin/setfacl"
  "usr.bin/ctlstat"
  "usr.sbin/mergemaster"
  "lib/libblocksruntime"
  "tests/sys/mqueue"
  "usr.sbin/makefs/tests"
  "sbin/devd/tests"
  "lib/libarchive/tests"
  "tests/sys/net"
  "libexec/tcpd"
  "usr.sbin/mfiutil"
  "sbin/hastd"
  "libexec/tftpd"
  "usr.sbin/mixer"
  "usr.bin/cut"
  "bin/sleep"
  "tests/sys/netgraph"
  "lib/libbsdstat"
  "sbin/rtsol"
  "tests/sys/net/routing"
  "usr.sbin/mlxcontrol"
  "usr.sbin/mountd"
  "libexec/tftpd/tests"
  "usr.sbin/mount_smbfs"
  "sbin/ipf"
  "usr.bin/cut/tests"
  "tests/sys/netinet"
  "bin/sleep/tests"
  "usr.sbin/mixer/tests"
  "usr.sbin/mpsutil"
  "sbin/ipf/libipf"
  "bin/sh/tests"
  "sbin/ipfw"
  "lib/libbsm"
  "usr.sbin/mptutil"
  "usr.sbin/mtest"
  "tests/sys/netinet6"
  "tests/sys/netipsec"
  "bin/stty"
  "usr.bin/diff"
  "tests/sys/netinet/libalias"
  "usr.sbin/newsyslog"
  "usr.sbin/nfscbd"
  "usr.sbin/nfsd"
  "bin/sh/tests/builtins"
  "usr.sbin/nfsdumpstate"
  "sbin/natd"
  "sbin/ipf/ipf"
  "usr.bin/diff/tests"
  "bin/sync"
  "tests/sys/netinet6/frag6"
  "lib/libbz2"
  "tests/sys/netipsec/tunnel"
  "tests/sys/netmap"
  "usr.sbin/newsyslog/newsyslog.conf.d"
  "usr.sbin/nfsrevoke"
  "usr.sbin/nfsuserd"
  "usr.bin/dirname"
  "sbin/iscontrol"
  "sbin/ipf/ipfs"
  "tests/sys/netpfil"
  "bin/test"
  "usr.sbin/newsyslog/tests"
  "usr.sbin/nmtree"
  "usr.bin/du"
  "lib/libcalendar"
  "usr.bin/dirname/tests"
  "tests/sys/opencrypto"
  "share/ctypedef"
  "sbin/decryptcore"
  "usr.sbin/nologin"
  "bin/test/tests"
  "sbin/ipf/ipfstat"
  "usr.sbin/nmtree/tests"
  "usr.bin/elfctl"
  "tests/sys/netpfil/pf"
  "sbin/pfctl"
  "usr.sbin/pciconf"
  "lib/libcam"
  "tests/sys/posixshm"
  "usr.bin/du/tests"
  "bin/sh/tests/errors"
  "usr.sbin/periodic"
  "bin/uuidgen"
  "usr.bin/elfdump"
  "share/dict"
  "usr.sbin/pnfsdscopymr"
  "sbin/ipf/ipmon"
  "sbin/pfctl/tests"
  "tests/sys/sys"
  "usr.sbin/pnfsdsfile"
  "tests/sys/netpfil/pf/ioctl"
  "usr.sbin/periodic/etc"
  "usr.bin/enigma"
  "lib/libcam/tests"
  "usr.bin/env"
  "usr.sbin/periodic/etc/daily"
  "usr.sbin/pnfsdskill"
  "usr.sbin/powerd"
  "bin/rmail"
  "bin/sh/tests/execution"
  "sbin/pfctl/tests/files"
  "share/doc"
  "lib/libcapsicum"
  "sbin/ipf/ipnat"
  "tests/sys/vfs"
  "usr.sbin/prometheus_sysctl_exporter"
  "share/doc/IPv6"
  "usr.bin/etdump"
  "tests/sys/netpfil/common"
  "usr.sbin/pstat"
  "sbin/pflogd"
  "lib/libcasper"
  "usr.bin/expand"
  "usr.sbin/periodic/etc/security"
  "sbin/ipf/ippool"
  "tests/sys/vm"
  "lib/libcasper/libcasper"
  "lib/libcompat"
  "lib/libcrypt"
  "usr.sbin/pw"
  "usr.sbin/pwd_mkdb"
  "bin/sh/tests/expansion"
  "share/doc/atf"
  "tests/sys/vmm"
  "usr.bin/false"
  "sbin/quotacheck"
  "sbin/routed"
  "usr.sbin/periodic/etc/weekly"
  "sbin/tests"
  "lib/libdevctl"
  "share/doc/legal"
  "lib/libcasper/services"
  "usr.sbin/pwm"
  "share/doc/pjdfstest"
  "lib/libc/tests"
  "bin/tests"
  "lib/libcrypt/tests"
  "usr.sbin/pw/tests"
  "bin/sh/tests/invocation"
  "share/doc/legal/intel_ipw"
  "usr.bin/fetch"
  "usr.sbin/periodic/etc/monthly"
  "sbin/routed/rtquery"
  "lib/libcasper/services/cap_dns"
  "share/examples"
  "lib/libdevinfo"
  "usr.sbin/quot"
  "lib/libdevstat"
  "usr.sbin/rarpd"
  "usr.sbin/rmt"
  "lib/libcasper/services/cap_fileargs"
  "share/i18n"
  "bin/sh/tests/parameters"
  "usr.sbin/rpcbind"
  "share/doc/legal/intel_iwi"
  "usr.bin/find"
  "lib/libdl"
  "share/i18n/csmapper"
  "usr.bin/fmt"
  "usr.sbin/rpc.lockd"
  "lib/libcasper/services/cap_dns/tests"
  "lib/libdwarf"
  "lib/libc/tests/tls_dso"
  "usr.sbin/rpc.statd"
  "lib/libedit"
  "usr.sbin/rpcbind/tests"
  "usr.sbin/rpc.umntall"
  "share/doc/legal/intel_iwlwififw"
  "lib/libcasper/services/cap_fileargs/tests"
  "bin/sh/tests/parser"
  "lib/libcasper/tests"
  "usr.bin/find/tests"
  "usr.sbin/rtprio"
  "lib/libc/tests/c063"
  "lib/libcasper/services/cap_grp"
  "share/i18n/csmapper/APPLE"
  "share/keys"
  "usr.sbin/rwhod"
  "lib/libedit/readline"
  "lib/libdwarf/sys"
  "share/keys/pkg"
  "share/doc/legal/intel_iwn"
  "lib/libcasper/services/cap_net"
  "lib/libelftc"
  "usr.sbin/service"
  "share/keys/pkg/trusted"
  "share/man"
  "share/doc/legal/intel_wpi"
  "usr.bin/fold"
  "lib/libc/tests/db"
  "usr.sbin/services_mkdb"
  "lib/libcasper/services/cap_grp/tests"
  "share/i18n/csmapper/AST"
  "lib/libevent1"
  "lib/libelftc/sys"
  "share/misc"
  "share/man/man1"
  "usr.sbin/sesutil"
  "share/mk"
  "usr.sbin/setfib"
  "lib/libexecinfo"
  "lib/libcasper/services/cap_net/tests"
  "share/doc/legal/realtek"
  "usr.bin/fold/tests"
  "bin/sh/tests/set-e"
  "lib/libcasper/services/cap_pwd"
  "lib/libc/tests/gen"
  "share/i18n/csmapper/BIG5"
  "usr.sbin/setfmac"
  "lib/libexecinfo/tests"
  "lib/libcasper/services/cap_sysctl"
  "share/i18n/esdb"
  "share/man/man3"
  "lib/libexpat"
  "usr.bin/fstat"
  "usr.sbin/setpmac"
  "usr.sbin/smbmsg"
  "usr.sbin/snapinfo"
  "share/monetdef"
  "lib/libcasper/services/cap_pwd/tests"
  "share/i18n/csmapper/CNS"
  "usr.sbin/spi"
  "lib/libfetch"
  "lib/libcasper/services/cap_sysctl/tests"
  "lib/libc/tests/gen/execve"
  "usr.sbin/syslogd"
  "usr.sbin/spray"
  "usr.sbin/sysrc"
  "lib/libfigpar"
  "usr.bin/fsync"
  "share/i18n/esdb/APPLE"
  "share/man/man3lua"
  "usr.sbin/tcpdrop"
  "lib/libcasper/services/cap_syslog"
  "lib/libc/tests/gen/posix_spawn"
  "usr.sbin/tcpdump"
  "usr.sbin/traceroute"
  "lib/libcasper/services/tests"
  "lib/libc/tests/hash"
  "share/i18n/csmapper/CP"
  "usr.sbin/trim"
  "usr.sbin/tcpdump/tcpdump"
  "usr.bin/gcore"
  "usr.sbin/trpt"
  "share/man/man4"
  "lib/libgeom"
  "share/msgdef"
  "usr.sbin/tzsetup"
  "lib/libifconfig"
  "usr.sbin/ugidfw"
  "lib/libc/tests/inet"
  "lib/libc/tests/net"
  "share/i18n/esdb/AST"
  "usr.bin/gencat"
  "usr.sbin/valectl"
  "lib/libipsec"
  "lib/libjail"
  "usr.sbin/vigr"
  "lib/libkiconv"
  "usr.sbin/vipw"
  "lib/libc/tests/nss"
  "usr.bin/getaddrinfo"
  "usr.sbin/wake"
  "lib/libc/tests/regex"
  "lib/libkvm"
  "share/i18n/csmapper/EBCDIC"
  "usr.sbin/watch"
  "share/i18n/esdb/BIG5"
  "lib/liblua"
  "share/man/man5"
  "lib/liblzma"
  "usr.sbin/watchdogd"
  "usr.sbin/zic"
  "lib/libc/tests/resolv"
  "usr.sbin/zonectl"
  "usr.sbin/zic/zic"
  "usr.bin/getconf"
  "usr.sbin/ofwdump"
  "share/i18n/csmapper/GB"
  "share/i18n/csmapper/GEORGIAN"
  "share/man/man6"
  "lib/libkvm/tests"
  "lib/libc/tests/rpc"
  "lib/libmemstat"
  "lib/libc/tests/setjmp"
  "usr.sbin/accton"
  "share/i18n/esdb/CP"
  "lib/libmd"
  "usr.sbin/zic/zdump"
  "share/i18n/csmapper/ISO646"
  "lib/libc/tests/stdio"
  "usr.bin/getconf/tests"
  "usr.sbin/sa"
  "share/man/man7"
  "share/i18n/csmapper/ISO-8859"
  "lib/libmt"
  "lib/libc/tests/stdlib"
  "lib/libc/tests/string"
  "usr.sbin/audit"
  "lib/libmd/sys"
  "usr.sbin/auditdistd"
  "usr.sbin/auditd"
  "usr.sbin/auditreduce"
  "share/man/man8"
  "share/i18n/csmapper/JIS"
  "usr.bin/getent"
  "usr.sbin/sa/tests"
  "share/i18n/csmapper/KAZAKH"
  "share/i18n/csmapper/KOI"
  "lib/libc/tests/stdlib/dynthr_mod"
  "usr.sbin/praudit"
  "usr.sbin/authpf"
  "lib/libc/tests/sys"
  "usr.sbin/autofs"
  "usr.bin/getopt"
  "share/numericdef"
  "usr.sbin/blacklistctl"
  "share/man/man9"
  "lib/libc/tests/termios"
  "share/i18n/csmapper/KS"
  "usr.bin/grep"
  "usr.sbin/blacklistd"
  "usr.sbin/praudit/tests"
  "share/i18n/csmapper/MISC"
  "usr.sbin/bluetooth"
  "usr.bin/gzip"
  "lib/libc/tests/time"
  "lib/libc/tests/tls"
  "usr.sbin/autofs/autofs"
  "share/sendmail"
  "usr.sbin/bootparamd"
  "share/i18n/csmapper/TCVN"
  "usr.sbin/bluetooth/bluetooth-config"
  "usr.sbin/bsdinstall"
  "usr.bin/grep/tests"
  "usr.sbin/bootparamd/bootparamd"
  "usr.sbin/bsnmpd"
  "usr.sbin/bootparamd/callbootd"
  "usr.bin/gzip/tests"
  "usr.sbin/bsnmpd/gensnmptree"
  "lib/libc/tests/ttyio"
  "share/skel"
  "lib/libc/tests/tls/dso"
  "usr.sbin/bluetooth/btpand"
  "usr.sbin/bluetooth/hccontrol"
  "usr.bin/head"
  "usr.sbin/certctl"
  "usr.sbin/bsdinstall/distextract"
  "share/snmp"
  "usr.sbin/bsdconfig"
  "usr.sbin/bsnmpd/bsnmpd"
  "usr.bin/hexdump"
  "usr.sbin/bsnmpd/modules"
  "usr.sbin/bluetooth/hcsecd"
  "share/snmp/mibs"
  "lib/libc/tests/iconv"
  "lib/libc/tests/locale"
  "usr.sbin/efivar"
  "usr.sbin/bsdinstall/distfetch"
  "usr.sbin/bluetooth/hcseriald"
  "share/syscons/fonts"
  "usr.bin/head/tests"
  "share/i18n/esdb/DEC"
  "usr.sbin/bsnmpd/tools"
  "share/tabset"
  "usr.sbin/efidp"
  "usr.sbin/bsdinstall/partedit"
  "usr.bin/hexdump/tests"
  "usr.sbin/bluetooth/l2control"
  "usr.sbin/bsnmpd/tools/libbsnmptools"
  "usr.sbin/bsdconfig/console"
  "lib/libc/tests/ssp"
  "usr.sbin/bsnmpd/modules/snmp_bridge"
  "share/termcap"
  "usr.sbin/bluetooth/l2ping"
  "lib/lib80211"
  "usr.bin/id"
  "share/syscons/keymaps"
  "usr.sbin/bluetooth/rfcomm_pppd"
  "usr.sbin/bsnmpd/tools/bsnmptools"
  "usr.sbin/bluetooth/sdpcontrol"
  "share/timedef"
  "share/i18n/esdb/EUC"
  "usr.sbin/bsdinstall/scripts"
  "usr.sbin/bsdconfig/console/include"
  "usr.sbin/bsnmpd/modules/snmp_hostres"
  "usr.bin/ident"
  "share/zoneinfo"
  "share/vt"
  "usr.sbin/bluetooth/sdpd"
  "lib/libnetbsd"
  "usr.bin/ipcrm"
  "usr.sbin/bluetooth/ath3kfw"
  "share/vt/fonts"
  "usr.sbin/bluetooth/bcmfw"
  "share/tests"
  "usr.sbin/bsnmpd/modules/snmp_lm75"
  "usr.sbin/efibootmgr"
  "usr.sbin/bsdconfig/diskmgmt"
  "usr.bin/ident/tests"
  "share/i18n/esdb/EBCDIC"
  "share/zoneinfo/tests"
  "share/examples/ipfilter"
  "usr.bin/ipcs"
  "usr.sbin/bluetooth/bthidcontrol"
  "usr.sbin/bluetooth/bthidd"
  "lib/libnetmap"
  "share/vt/keymaps"
  "usr.sbin/bluetooth/iwmbtfw"
  "usr.sbin/bsnmpd/modules/snmp_mibII"
  "usr.sbin/bsdconfig/diskmgmt/include"
  "usr.sbin/uefisign"
  "usr.sbin/bsnmpd/modules/snmp_target"
  "usr.sbin/fdcontrol"
  "usr.sbin/fdformat"
  "usr.bin/join"
  "share/examples/smbfs"
  "share/examples/tests"
  "lib/libnv"
  "usr.bin/jot"
  "lib/libopenbsd"
  "share/i18n/esdb/GB"
  "share/i18n/esdb/GEORGIAN"
  "usr.sbin/bsdconfig/docsinstall"
  "usr.sbin/fdread"
  "usr.bin/join/tests"
  "share/examples/smbfs/print"
  "usr.sbin/bsnmpd/modules/snmp_usm"
  "usr.bin/keylogin"
  "usr.sbin/fdwrite"
  "usr.bin/jot/tests"
  "share/examples/tests/tests"
  "lib/libnv/tests"
  "usr.sbin/fmtree"
  "usr.sbin/bsnmpd/modules/snmp_vacm"
  "lib/libopie"
  "share/i18n/esdb/ISO-2022"
  "usr.sbin/freebsd-update"
  "share/i18n/esdb/ISO-8859"
  "usr.bin/keylogout"
  "usr.bin/killall"
  "usr.sbin/gssd"
  "usr.sbin/bsnmpd/modules/snmp_hast"
  "usr.sbin/bsdconfig/docsinstall/include"
  "share/examples/tests/tests/atf"
  "lib/libpam"
  "lib/libpathconv"
  "lib/libpam/libpam"
  "usr.sbin/bsnmpd/modules/snmp_pf"
  "usr.sbin/gpioctl"
  "usr.sbin/bsnmpd/modules/snmp_netgraph"
  "usr.sbin/ip6addrctl"
  "usr.bin/ktrace"
  "usr.bin/ktrdump"
  "share/examples/tests/tests/plain"
  "share/examples/tests/tests/tap"
  "usr.sbin/bsnmpd/modules/snmp_wlan"
  "share/i18n/esdb/ISO646"
  "lib/libpcap"
  "usr.sbin/mld6query"
  "share/i18n/esdb/KAZAKH"
  "usr.bin/lam"
  "usr.sbin/ndp"
  "usr.bin/ldd"
  "share/examples/tests/tests/googletest"
  "share/i18n/esdb/KOI"
  "lib/libpam/modules"
  "usr.sbin/rip6query"
  "usr.sbin/route6d"
  "usr.sbin/bsdconfig/dot"
  "usr.sbin/rrenumd"
  "usr.sbin/bsdconfig/include"
  "usr.bin/leave"
  "lib/libpjdlog"
  "share/i18n/esdb/MISC"
  "lib/libpam/modules/pam_chroot"
  "lib/libproc"
  "share/i18n/esdb/TCVN"
  "usr.bin/less"
  "usr.sbin/bsdconfig/includes"
  "usr.sbin/rtadvctl"
  "usr.sbin/rtadvd"
  "usr.sbin/bsdconfig/dot/include"
  "usr.sbin/bsdconfig/mouse"
  "share/i18n/esdb/UTF"
  "usr.sbin/rtsold"
  "lib/libpam/modules/pam_deny"
  "lib/libpam/pam.d"
  "lib/libpam/modules/pam_echo"
  "lib/libpam/static_libpam"
  "lib/libproc/tests"
  "usr.sbin/traceroute6"
  "usr.sbin/inetd"
  "usr.sbin/bsdconfig/includes/include"
  "usr.bin/lessecho"
  "usr.sbin/bsdconfig/networking"
  "usr.sbin/bsdconfig/mouse/include"
  "usr.bin/lesskey"
  "usr.sbin/bsdconfig/packages"
  "usr.sbin/ipfwpcap"
  "lib/libpam/modules/pam_exec"
  "usr.sbin/bsdconfig/password"
  "usr.sbin/bsdconfig/security"
  "usr.sbin/ctladm"
  "usr.sbin/ctld"
  "usr.sbin/jail"
  "usr.sbin/iscsid"
  "usr.bin/limits"
  "usr.sbin/bsdconfig/share"
  "usr.sbin/bsdconfig/networking/include"
  "usr.sbin/bsdconfig/startup"
  "usr.sbin/bsdconfig/timezone"
  "usr.sbin/bsdconfig/packages/include"
  "usr.bin/locale"
  "usr.sbin/jexec"
  "usr.sbin/bsdconfig/security/include"
  "usr.sbin/bsdconfig/password/include"
  "lib/libpam/modules/pam_ftpusers"
  "usr.sbin/bsdconfig/password/share"
  "usr.bin/limits/tests"
  "usr.sbin/jls"
  "usr.sbin/bsdconfig/share/media"
  "usr.sbin/bsdconfig/startup/include"
  "usr.sbin/jail/tests"
  "usr.sbin/kbdcontrol"
  "usr.sbin/bsdconfig/networking/share"
  "usr.sbin/bsdconfig/timezone/include"
  "usr.sbin/bsdconfig/ttys"
  "usr.sbin/kbdmap"
  "usr.bin/locale/tests"
  "usr.sbin/bsdconfig/startup/share"
  "usr.sbin/moused"
  "lib/libpam/modules/pam_group"
  "lib/libprocstat"
  "usr.bin/localedef"
  "usr.sbin/vidcontrol"
  "usr.sbin/pppctl"
  "usr.sbin/nscd"
  "usr.bin/lock"
  "usr.sbin/bsdconfig/share/packages"
  "usr.sbin/bsdconfig/timezone/share"
  "usr.sbin/bsdconfig/usermgmt"
  "usr.sbin/bsdconfig/ttys/include"
  "usr.sbin/lpr"
  "usr.sbin/manctl"
  "lib/libregex"
  "lib/libpam/modules/pam_guest"
  "usr.bin/lockf"
  "usr.sbin/bsdconfig/examples"
  "usr.bin/logger"
  "usr.sbin/lpr/common_source"
  "usr.bin/login"
  "usr.bin/logins"
  "usr.sbin/flowctl"
  "usr.sbin/bsdconfig/usermgmt/include"
  "usr.sbin/bsdconfig/usermgmt/share"
  "usr.sbin/ngctl"
  "usr.sbin/lpr/chkprintcap"
  "usr.bin/logname"
  "usr.bin/look"
  "usr.sbin/nghook"
  "usr.sbin/lpr/lp"
  "usr.bin/lorder"
  "usr.sbin/ntp"
  "lib/libregex/tests"
  "usr.sbin/keyserv"
  "usr.bin/lsvfs"
  "lib/libpam/modules/pam_krb5"
  "lib/librpcsvc"
  "usr.sbin/ftp-proxy"
  "usr.sbin/pkg"
  "usr.sbin/ntp/libopts"
  "release"
  "usr.bin/lzmainfo"
  "lib/libpam/modules/pam_ksu"
  "usr.sbin/lpr/lpc"
  "usr.sbin/pmc"
  "usr.sbin/lpr/lpd"
  "lib/librss"
  "usr.bin/m4"
  "lib/librt"
  "lib/librtld_db"
  "usr.sbin/pmcannotate"
  "usr.sbin/pmccontrol"
  "lib/libpam/modules/pam_lastlog"
  "usr.sbin/lpr/lpq"
  "lib/libpam/modules/pam_login_access"
  "usr.sbin/ntp/libntp"
  "usr.bin/mandoc"
  "usr.sbin/pmcstat"
  "lib/librt/tests"
  "lib/libsbuf"
  "usr.bin/m4/tests"
  "usr.sbin/lpr/lpr"
  "usr.sbin/pmcstudy"
  "usr.sbin/lpr/lprm"
  "usr.sbin/portsnap"
  "lib/libpam/modules/pam_opie"
  "usr.sbin/lpr/lptest"
  "lib/libpam/modules/pam_nologin"
  "usr.sbin/ppp"
  "usr.sbin/portsnap/portsnap"
  "usr.sbin/portsnap/make_index"
  "lib/libsbuf/tests"
  "lib/libsmb"
  "usr.sbin/lpr/pac"
  "lib/libpam/modules/pam_opieaccess"
  "usr.bin/mesg"
  "usr.sbin/edquota"
  "usr.bin/minigzip"
  "lib/libpam/modules/pam_passwdqc"
  "usr.sbin/ntp/libntpevent"
  "lib/libpam/modules/pam_permit"
  "lib/libstdbuf"
  "usr.sbin/lpr/filters"
  "usr.bin/ministat"
  "usr.sbin/lpr/filters.ru"
  "usr.sbin/quotaon"
  "lib/libstdthreads"
  "usr.bin/mkdep"
  "lib/libsysdecode"
  "usr.sbin/repquota"
  "lib/libpam/modules/pam_radius"
  "usr.sbin/editmap"
  "lib/libpam/modules/pam_rhosts"
  "usr.sbin/ntp/libparse"
  "usr.sbin/mailstats"
  "usr.bin/mkimg"
  "lib/libpam/modules/pam_rootok"
  "usr.bin/mkfifo"
  "lib/libpam/modules/pam_securetty"
  "lib/libtacplus"
  "lib/libthr"
  "lib/libthread_db"
  "usr.sbin/lpr/filters.ru/koi2alt"
  "usr.sbin/makemap"
  "usr.sbin/lpr/filters.ru/koi2855"
  "usr.sbin/ntp/ntpd"
  "usr.sbin/praliases"
  "lib/libpam/modules/pam_self"
  "lib/libpam/modules/pam_ssh"
  "usr.sbin/sendmail"
  "usr.bin/mktemp"
  "lib/libpam/modules/pam_tacplus"
  "lib/libpam/modules/pam_unix"
  "lib/libucl"
  "usr.sbin/tcpdchk"
  "usr.bin/mkimg/tests"
  "lib/libthr/tests"
  "lib/libufs"
  "lib/libugidfw"
  "usr.sbin/tcpdmatch"
  "lib/libutil"
  "lib/libulog"
  "lib/libwrap"
  "usr.sbin/ntp/ntpdc"
  "usr.bin/mkuzip"
  "usr.sbin/config"
  "usr.sbin/crunch"
  "lib/libxo"
  "lib/liby"
  "lib/libz"
  "lib/libthr/tests/dlopen"
  "usr.sbin/unbound"
  "lib/libzstd"
  "usr.sbin/crunch/crunchgen"
  "lib/libutil/tests"
  "lib/libxo/libxo"
  "usr.sbin/uathload"
  "usr.sbin/crunch/crunchide"
  "usr.sbin/ntp/ntpq"
  "usr.sbin/unbound/daemon"
  "lib/flua"
  "usr.bin/mt"
  "lib/libngatm"
  "lib/flua/libjail"
  "lib/libthr/tests/dlopen/dso"
  "lib/libxo/encoder"
  "lib/libblacklist"
  "usr.sbin/uhsoctl"
  "lib/libbluetooth"
  "usr.sbin/ntp/ntpdate"
  "lib/libxo/tests"
  "usr.sbin/usbconfig"
  "lib/libxo/encoder/csv"
  "usr.sbin/ntp/ntptime"
  "usr.sbin/unbound/anchor"
  "usr.bin/ncal"
  "usr.bin/netstat"
  "lib/libsdp"
  "lib/libbsnmp"
  "usr.sbin/usbdump"
  "lib/libcuse"
  "lib/libdevdctl"
  "lib/libbsnmp/libbsnmp"
  "usr.sbin/ac"
  "usr.sbin/ntp/ntp-keygen"
  "lib/libpe"
  "usr.bin/ncal/tests"
  "usr.sbin/ntp/sntp"
  "usr.bin/newgrp"
  "usr.bin/nfsstat"
  "usr.sbin/lastlogin"
  "usr.sbin/unbound/checkconf"
  "lib/libdpv"
  "usr.sbin/utx"
  "lib/libgpio"
  "usr.sbin/ancontrol"
  "usr.bin/nice"
  "lib/libbsnmp/tests"
  "usr.sbin/wlandebug"
  "usr.sbin/wpa"
  "usr.sbin/ntp/doc"
  "lib/ncurses/form"
  "usr.sbin/unbound/control"
  "usr.bin/nl"
  "usr.sbin/wpa/src"
  "usr.sbin/unbound/setup"
  "lib/libgssapi"
  "usr.sbin/tests"
  "lib/librpcsec_gss"
  "usr.bin/nohup"
  "lib/libiconv_modules"
  "usr.bin/opieinfo"
  "lib/libxo/tests/encoder"
  "lib/libcom_err"
  "usr.bin/opiekey"
  "usr.sbin/wpa/src/ap"
  "lib/libldns"
  "usr.sbin/wpa/src/common"
  "usr.bin/opiepasswd"
  "usr.sbin/wpa/wpa_supplicant"
  "lib/libiconv_modules/BIG5"
  "lib/libstats"
  "lib/ncurses/menu"
  "usr.sbin/ntp/doc/drivers"
  "lib/libiconv_modules/DECHanyu"
  "lib/libefivar"
  "usr.bin/pagesize"
  "usr.sbin/ntp/doc/hints"
  "lib/googletest"
  "lib/libnetgraph"
  "usr.bin/passwd"
  "lib/ncurses/panel"
  "usr.bin/paste"
  "usr.sbin/wpa/src/crypto"
  "usr.sbin/wpa/src/drivers"
  "lib/libiconv_modules/EUC"
  "usr.sbin/wpa/wpa_cli"
  "lib/googletest/gtest"
  "lib/libomp"
  "lib/libiconv_modules/EUCTW"
  "lib/libcbor"
  "usr.bin/patch"
  "usr.sbin/wpa/src/eap_common"
  "usr.bin/pathchk"
  "usr.sbin/wpa/src/eap_peer"
  "usr.sbin/ntp/doc/drivers/icons"
  "lib/libfido2"
  "usr.sbin/ntp/doc/icons"
  "usr.bin/perror"
  "usr.sbin/wpa/wpa_passphrase"
  "lib/libiconv_modules/GBK2K"
  "usr.sbin/wpa/src/eap_server"
  "lib/libiconv_modules/HZ"
  "lib/libiconv_modules/ISO2022"
  "lib/libiconv_modules/JOHAB"
  "lib/libfido2/openbsd-compat"
  "usr.sbin/wpa/src/eapol_auth"
  "usr.sbin/wpa/src/eapol_supp"
  "lib/libcbor/cbor"
  "usr.bin/posixshmcontrol"
  "lib/libcbor/cbor/internal"
  "lib/googletest/gmock"
  "usr.bin/pr"
  "lib/libmp"
  "usr.sbin/wpa/hostapd"
  "usr.sbin/wpa/hostapd_cli"
  "usr.sbin/ntp/doc/drivers/scripts"
  "usr.bin/patch/tests"
  "lib/libpfctl"
  "lib/libiconv_modules/MSKanji"
  "lib/libiconv_modules/UES"
  "usr.sbin/wpa/src/l2_packet"
  "lib/libiconv_modules/UTF1632"
  "lib/libiconv_modules/UTF7"
  "usr.sbin/wpa/src/radius"
  "lib/libmp/tests"
  "usr.sbin/wpa/src/rsn_supp"
  "usr.bin/printenv"
  "usr.bin/pr/tests"
  "usr.sbin/ntp/doc/pic"
  "usr.sbin/wpa/src/tls"
  "usr.sbin/wpa/ndis_events"
  "lib/libpmcstat"
  "lib/googletest/gmock_main"
  "usr.sbin/wpa/src/utils"
  "lib/libiconv_modules/UTF8"
  "lib/libradius"
  "usr.sbin/wpa/src/wps"
  "usr.bin/printf"
  "lib/libmilter"
  "lib/libsm"
  "usr.bin/procstat"
  "usr.bin/proccontrol"
  "lib/libiconv_modules/VIQR"
  "lib/libsmdb"
  "usr.bin/protect"
  "lib/libsmutil"
  "lib/googletest/gtest_main"
  "usr.bin/rctl"
  "usr.bin/renice"
  "lib/libtelnet"
  "usr.bin/printf/tests"
  "lib/atf"
  "lib/libiconv_modules/ZW"
  "usr.sbin/ntp/doc/scripts"
  "usr.bin/procstat/tests"
  "lib/atf/libatf-c"
  "lib/tests"
  "lib/libunbound"
  "usr.bin/resizewin"
  "lib/libusbhid"
  "usr.bin/rev"
  "usr.bin/revoke"
  "lib/googletest/tests"
  "lib/libusb"
  "usr.bin/rpcinfo"
  "lib/libiconv_modules/iconv_none"
  "usr.bin/rs"
  "usr.bin/rup"
  "usr.bin/ruptime"
  "usr.bin/rusers"
  "usr.bin/rwall"
  "lib/ofed"
  "usr.bin/rwho"
  "lib/googletest/tests/gmock_main"
  "usr.bin/script"
  "lib/ofed/include"
  "lib/liblutok"
  "lib/ofed/libibverbs"
  "lib/atf/libatf-c/tests"
  "lib/atf/libatf-c++"
  "usr.bin/sdiff"
  "lib/atf/tests"
  "usr.bin/sed"
  "usr.bin/rs/tests"
  "lib/libiconv_modules/iconv_std"
  "lib/libiconv_modules/mapper_646"
  "usr.bin/seq"
  "usr.bin/shar"
  "lib/googletest/tests/gmock"
  "lib/googletest/tests/gtest_main"
  "usr.bin/sdiff/tests"
  "usr.bin/showmount"
  "lib/atf/tests/test-programs"
  "usr.bin/sockstat"
  "lib/ofed/libibcm"
  "lib/atf/libatf-c++/tests"
  "lib/atf/libatf-c/tests/detail"
  "lib/googletest/tests/gtest"
  "lib/libiconv_modules/mapper_none"
  "usr.bin/sed/tests"
  "usr.bin/seq/tests"
  "lib/libiconv_modules/mapper_parallel"
  "lib/ofed/libibumad"
  "lib/ofed/librdmacm"
  "lib/ofed/libibmad"
  "lib/ofed/complib"
  "usr.bin/soelim"
  "usr.bin/sort"
  "usr.bin/split"
  "usr.bin/stat"
  "lib/ofed/libvendor"
  "lib/libiconv_modules/mapper_serial"
  "usr.bin/stdbuf"
  "lib/ofed/libcxgb4"
  "lib/atf/libatf-c++/tests/detail"
  "lib/libiconv_modules/mapper_std"
  "lib/libiconv_modules/mapper_zone"
  "lib/ofed/libmlx4"
  "usr.bin/sed/tests/regress.multitest.out"
  "lib/ofed/libmlx5"
  "lib/ofed/libibnetdisc"
  "lib/ofed/libopensm"
  "usr.bin/soelim/tests"
  "usr.bin/sort/tests"
  "usr.bin/strings"
  "usr.bin/systat"
  "usr.bin/su"
  "usr.bin/stat/tests"
  "usr.bin/tabs"
  "usr.bin/tail"
  "usr.bin/tar"
  "usr.bin/tcopy"
  "usr.bin/time"
  "usr.bin/tee"
  "usr.bin/timeout"
  "usr.bin/tip"
  "usr.bin/top"
  "usr.bin/tip/tip"
  "usr.bin/touch"
  "usr.bin/tput"
  "usr.bin/tr"
  "usr.bin/true"
  "usr.bin/truncate"
  "usr.bin/tsort"
  "usr.bin/tset"
  "usr.bin/tail/tests"
  "usr.bin/tty"
  "usr.bin/tar/tests"
  "usr.bin/uname"
  "usr.bin/unexpand"
  "usr.bin/uniq"
  "usr.bin/timeout/tests"
  "usr.bin/unzip"
  "usr.bin/units"
  "usr.bin/tr/tests"
  "usr.bin/unvis"
  "usr.bin/uudecode"
  "usr.bin/uuencode"
  "usr.bin/truncate/tests"
  "usr.bin/vis"
  "usr.bin/vmstat"
  "usr.bin/w"
  "usr.bin/wall"
  "usr.bin/what"
  "usr.bin/wc"
  "usr.bin/whereis"
  "usr.bin/uniq/tests"
  "usr.bin/which"
  "usr.bin/whois"
  "usr.bin/units/tests"
  "usr.bin/write"
  "usr.bin/xargs"
  "usr.bin/uudecode/tests"
  "usr.bin/xinstall"
  "usr.bin/xo"
  "usr.bin/vmstat/tests"
  "usr.bin/uuencode/tests"
  "usr.bin/xz"
  "usr.bin/xzdec"
  "usr.bin/yes"
  "usr.bin/zstd"
  "usr.bin/lastcomm"
  "usr.bin/gh-bc"
  "usr.bin/at"
  "usr.bin/atm"
  "usr.bin/bluetooth"
  "usr.bin/cpio"
  "usr.bin/xo/tests"
  "usr.bin/atm/sscop"
  "usr.bin/xargs/tests"
  "usr.bin/xinstall/tests"
  "usr.bin/dpv"
  "usr.bin/calendar"
  "usr.bin/ee"
  "usr.bin/file"
  "usr.bin/bluetooth/bthost"
  "usr.bin/ftp"
  "usr.bin/lastcomm/tests"
  "usr.bin/finger"
  "usr.bin/caesar"
  "usr.bin/cpio/tests"
  "usr.bin/factor"
  "usr.bin/fortune"
  "usr.bin/grdc"
  "usr.bin/morse"
  "usr.bin/calendar/tests"
  "usr.bin/number"
  "usr.bin/fortune/fortune"
  "usr.bin/pom"
  "usr.bin/fortune/strfile"
  "usr.bin/primes"
  "usr.bin/random"
  "usr.bin/bluetooth/btsockstat"
  "usr.bin/file/tests"
  "usr.bin/dtc"
  "usr.bin/fortune/datfiles"
  "usr.bin/gh-bc/tests"
  "usr.bin/iconv"
  "usr.bin/mkcsmapper"
  "usr.bin/mkesdb"
  "usr.bin/iscsictl"
  "usr.bin/kdump"
  "usr.bin/truss"
  "usr.bin/fortune/unstr"
  "usr.bin/compile_et"
  "usr.bin/drill"
  "usr.bin/host"
  "usr.bin/bluetooth/rfcomm_sppd"
  "usr.bin/locate"
  "usr.bin/biff"
  "usr.bin/from"
  "usr.bin/mail"
  "usr.bin/msgs"
  "usr.bin/locate/bigram"
  "usr.bin/bmake"
  "usr.bin/man"
  "usr.bin/nc"
  "usr.bin/ssh-copy-id"
  "usr.bin/chkey"
  "usr.bin/newkey"
  "usr.bin/quota"
  "usr.bin/vacation"
  "usr.bin/talk"
  "usr.bin/telnet"
  "usr.bin/locate/code"
  "usr.bin/tests"
  "usr.bin/ul"
  "usr.bin/tftp"
  "usr.bin/addr2line"
  "usr.bin/ar"
  "usr.bin/locate/locate"
  "usr.bin/nm"
  "usr.bin/objcopy"
  "usr.bin/bmake/tests"
  "usr.bin/readelf"
  "usr.bin/size"
  "usr.bin/c89"
  "usr.bin/c99"
  "usr.bin/file2c"
  "usr.bin/ctags"
  "usr.bin/gprof"
  "usr.bin/indent"
  "usr.bin/lex"
  "usr.bin/mkstr"
  "usr.bin/rpcgen"
  "usr.bin/unifdef"
  "usr.bin/xstr"
  "usr.bin/readelf/sys"
  "usr.bin/objcopy/sys"
  "usr.bin/yacc"
  "usr.bin/vtfontcvt"
  "usr.bin/usbhidaction"
  "usr.bin/bmake/tests/archives"
  "usr.bin/file2c/tests"
  "usr.bin/usbhidctl"
  "usr.bin/last"
  "usr.bin/users"
  "usr.bin/who"
  "usr.bin/ofed"
  "usr.bin/indent/tests"
  "usr.bin/lex/lib"
  "usr.bin/kyua"
  "usr.bin/unifdef/tests"
  "usr.bin/bmake/tests/basic"
  "usr.bin/yacc/tests"
  "usr.bin/ofed/librdmacm"
  "usr.bin/bmake/tests/execution"
  "usr.bin/ofed/libibverbs"
  "usr.bin/bmake/tests/archives/fmt_44bsd"
  "usr.bin/bmake/tests/shell"
  "usr.bin/bmake/tests/suffixes"
  "usr.bin/bmake/tests/syntax"
  "usr.bin/ofed/librdmacm/ucmatose"
  "usr.bin/bmake/tests/archives/fmt_44bsd_mod"
  "usr.bin/ofed/libibverbs/asyncwatch"
  "usr.bin/ofed/libibverbs/devinfo"
  "usr.bin/ofed/libibverbs/devices"
  "usr.bin/ofed/infiniband-diags"
  "usr.bin/bmake/tests/archives/fmt_oldbsd"
  "usr.bin/bmake/tests/shell/builtin"
  "usr.bin/bmake/tests/basic/t0"
  "usr.bin/bmake/tests/suffixes/basic"
  "usr.bin/bmake/tests/suffixes/src_wild1"
  "usr.bin/bmake/tests/execution/ellipsis"
  "usr.bin/ofed/librdmacm/mckey"
  "usr.bin/ofed/infiniband-diags/ibstat"
  "usr.bin/ofed/libibverbs/rc_pingpong"
  "usr.bin/ofed/libibverbs/srq_pingpong"
  "usr.bin/kyua/cli"
  "usr.bin/ofed/libibverbs/uc_pingpong"
  "usr.bin/kyua/drivers"
  "usr.bin/kyua/engine"
  "usr.bin/kyua/model"
  "usr.bin/kyua/store"
  "usr.bin/kyua/utils"
  "usr.bin/bmake/tests/syntax/directive-t0"
  "usr.bin/bmake/tests/shell/meta"
  "usr.bin/kyua/utils/cmdline"
  "usr.bin/kyua/utils/config"
  "usr.bin/kyua/utils/format"
  "usr.bin/kyua/utils/fs"
  "usr.bin/bmake/tests/suffixes/src_wild2"
  "usr.bin/kyua/utils/logging"
  "usr.bin/bmake/tests/shell/path"
  "usr.bin/bmake/tests/basic/t1"
  "usr.bin/kyua/utils/process"
  "usr.bin/kyua/utils/signals"
  "usr.bin/kyua/utils/sqlite"
  "usr.bin/kyua/utils/text"
  "usr.bin/bmake/tests/syntax/enl"
  "usr.bin/bmake/tests/sysmk"
  "usr.bin/bmake/tests/execution/empty"
  "usr.bin/ofed/librdmacm/rping"
  "usr.bin/ofed/librdmacm/udaddy"
  "usr.bin/ofed/libibverbs/ud_pingpong"
  "usr.bin/bmake/tests/syntax/funny-targets"
  "usr.bin/bmake/tests/basic/t2"
  "usr.bin/bmake/tests/variables"
  "usr.bin/bmake/tests/syntax/semi"
  "usr.bin/bmake/tests/execution/joberr"
  "usr.bin/bmake/tests/shell/path_select"
  "usr.bin/bmake/tests/shell/replace"
  "usr.bin/bmake/tests/basic/t3"
  "usr.bin/bmake/tests/shell/select"
  "usr.bin/bmake/tests/execution/plus"
  "usr.bin/bmake/tests/sysmk/t0"
  "usr.bin/bmake/tests/sysmk/t1"
  "usr.bin/bmake/tests/sysmk/t2"
  "usr.bin/bmake/tests/variables/modifier_M"
  "usr.bin/bmake/tests/variables/opt_V"
  "usr.bin/bmake/tests/variables/modifier_t"
  "usr.bin/bmake/tests/variables/t0"
  "usr.bin/bmake/tests/sysmk/t0/mk"
  "usr.bin/bmake/tests/sysmk/t0/2"
  "usr.bin/bmake/tests/sysmk/t1/mk"
  "usr.bin/bmake/tests/sysmk/t2/mk"
  "usr.bin/bmake/tests/sysmk/t1/2"
  "usr.bin/bmake/tests/sysmk/t2/2"
  "usr.bin/bmake/tests/sysmk/t0/2/1"
  "usr.bin/bmake/tests/sysmk/t1/2/1"
  "usr.bin/bmake/tests/sysmk/t2/2/1"
)

cd ${world_dir}

for element in ${install_objdir[@]}
do
  mkdir -p ${element}
done

cd ${root_path} 